...
void vetor_int(){
    ...
    #pragma omp parallel for reduction(+:vetorSumI)
    for(i = 0 ; i < TAM ; i++){
        vetor[i] = 22;
        vetorSumI = vetorSumI + vetor[i];  
        //cout<<".";
    }
    cout<<endl<< "soma do vetor de inteiros: " <<vetorSumI;
}
...
void vetor_float(){
    ...
    #pragma omp parallel for reduction(+:vetorSumF)
    for(i = 0 ; i < TAM ; i++){
        vetor[i] = 22.2;
        //vetor[i] = rand()%100;
        //vetor[i] = vetor[i]/100 + rand()%100;
        vetorSumF = vetorSumF + vetor[i];  
        //cout<<".";
    }
    cout<<endl<< "soma do vetor de floats: " <<vetorSumF;
}
